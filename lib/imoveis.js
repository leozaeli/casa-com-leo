...
export function formatPrice(preco) {
  if (!Number(preco)) return 'Sob consulta';
...
const SURROUNDINGS_RADIUS_M = 2000;
...
export function buildImovelSpecs(imovel) {
  const specs = [];
  const formatArea = (area) => Number(area).toLocaleString('pt-BR', { maximumFractionDigits: 2 });
  if (imovel.area_total_m2 > 0) specs.push({ value: `${formatArea(imovel.area_total_m2)} m²`, label: 'Área total' });
  if (imovel.area_m2 > 0) specs.push({ value: `${formatArea(imovel.area_m2)} m²`, label: 'Área construída' });
  if (imovel.quartos > 0) specs.push({ value: `${imovel.quartos}`, label: imovel.quartos === 1 ? 'Quarto' : 'Quartos' });
...
export async function listImoveis() {
  ...
  const { data, error } = await supabase
    .from('imoveis')
    .select('*')
    // Every registered property belongs in the public catalogue. `destaque`
    // only controls its position, so older entries that were not marked as a
    // highlight do not disappear from the home page or /imoveis.
  ...
  const launchPages = await listLaunchBriefs();
  const launchByProperty = new Map(
    launchPages
      .filter((launch) => launch.status === 'published' && launch.property_slug)
      .map((launch) => [launch.property_slug, launch])
  );
  return data.map((imovel) => {
    const launch = launchByProperty.get(imovel.slug);
    return launch ? { ...imovel, launch_url: 'https://' + launch.subdomain + '.casacomleo.com.br' } : imovel;
  });
}
