...
export async function resolveMapEmbedSrc(rawLocation) {
  const raw = (rawLocation || '').toString().trim();
  if (!raw) return null;

  const [addressPart, coordsPart] = raw.split('|');
  if (coordsPart && /^-?\d+(\.\d+)?,-?\d+(\.\d+)?$/.test(coordsPart.trim())) {
    return `https://www.google.com/maps?q=${coordsPart.trim()}&output=embed`;
  }
  ...
  if (/google\.com\/maps\/(embed|d\/embed)/i.test(target)) {
    return target;
  }
  ...
  return `https://www.google.com/maps?q=${encodeURIComponent(base)}&output=embed`;
}
