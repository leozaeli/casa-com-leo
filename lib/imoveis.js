...
export function buildImovelSpecs(imovel) {
  const specs = [];
  if (imovel.area_total_m2 > 0) specs.push({ value: `${imovel.area_total_m2} m²`, label: 'Área total' });
  if (imovel.area_m2 > 0) specs.push({ value: `${imovel.area_m2} m²`, label: 'Área construída' });
