...
export async function provisionLaunchDomain(subdomain) {
  ...
  if (!response.ok && response.status !== 409) {
    return { error: 'Não foi possível reservar o subdomínio no Vercel.' };
  }

  const verify = await fetch(`${endpoint}/${hostname}/verify`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}` },
    cache: 'no-store',
  });
  if (!verify.ok) return { error: 'O subdomínio foi reservado, mas o certificado ainda não pôde ser validado.' };
  return { hostname };
}
