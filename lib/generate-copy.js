...
export async function generateSurroundingsCopy({ localizacao, titulo, surroundings }) {
  ...
  if (surroundings.farmacias > 0) fatos.push(`${surroundings.farmacias} farmácia(s) num raio de 5 km.`);
  if (surroundings.supermercados > 0) fatos.push(`${surroundings.supermercados} mercado(s)/supermercado(s) num raio de 5 km.`);
...
Fatos sobre o entorno (raio de 5 km, coletados do OpenStreetMap):
${fatos.map((f) => `- ${f}`).join('\n')}
